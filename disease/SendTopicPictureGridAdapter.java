 ...
 * @author xiaoqing
 */
public class SendTopicPictureGridAdapter extends BaseAdapter {
    ...
    public SendTopicPictureGridAdapter(Context context,
    ...
    public View getView(int position, View convertView, final ViewGroup parent) {
        ...
        if (showType == 1) {
            ...
            if (position == listPicture.size() - 1) {
                vh.pictureCloseImg.setVisibility(View.GONE);
                if (context instanceof NewdiseaseActivity) {
                    vh.pictureImg.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            if (minePopupWindow == null) {
                                minePopupWindow = new MinePopupWindow((NewdiseaseActivity) context, itemOnClick);
                            }
                            minePopupWindow.showAtLocation(parent, Gravity.BOTTOM, 0, 0);
                            Utils.hideInputWindow((NewdiseaseActivity)context);
                        }
                    });
                }
            } else {
    ...
    public void getPicFromPhoto() {
        ...
        ((NewdiseaseActivity) context).startActivityForResult(intent, CHOOSE_PICTURE_CODE);
    }
    ...
    public void getPicFromCamera() {
        ...
        ((NewdiseaseActivity) context).setCameraPath(path);
        ((NewdiseaseActivity) context).setChildViewPosition(childViewPosition);
        ((NewdiseaseActivity) context).startActivityForResult(intent, CAMERA_CODE);
    }
