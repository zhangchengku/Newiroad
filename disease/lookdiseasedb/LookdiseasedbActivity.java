...
public class LookdiseasedbActivity extends MVPBaseActivity<LookdiseasedbContract.View, LookdiseasedbPresenter> implements LookdiseasedbContract.View {
    ...
    private LookdiseasedbGrid addPictureAdapter;
    private Map<Integer, LookdiseasedbGrid> mapAdapter = new HashMap<>();
    private List<RoadSectionInfo> lxResultdb;
    ...
    private List<String> dbGCXMlist = new ArrayList<>();
    private List<String> dbSLlist = new ArrayList<>();
    private List<String> dbDWlist = new ArrayList<>();
    private List<String> dbGCXMIDlist = new ArrayList<>();
    private int bhjbid;
    ...
    private RoadSectionInfo lxInfo;
    ...
    private void updata() {
        if (Utils.isNetworkAvailable(LookdiseasedbActivity.this) == false) {//没网
            ...
            if (result == 1) {
                ...
                adddiseasejson.setLXCODE(lxInfo.getLXID());
                String zhStr = "0.0";
                String zhOneStr = diseaseNewPileNumberOneEdit.getText().toString();
                String zhTwoStr = diseaseNewPileNumberTwoEdit.getText().toString();
                if (Utils.isNull(zhTwoStr)) {
                ...
                if (gcxmadapter.get() != null && gcxmadapter.get().size() > 0) {
                    for (int k = 0; k < gcxmadapter.get().size(); k++) {
                        ...
                        cZFABean.setJSGS(replaceNullS(gcxmadapter.get().get(k).getJsgs()));
    ...
    public void getData(adddiseasebean videoVos2) {
        if (videoVos2.getState().equals("1")) {
            ...
            curingDao.deleteBingHaiBaseById(bhjbid);
    ...
    public void getData2(Dingbean videoVos2) {
        if (videoVos2.getSTATE().equals("1") && videoVos2.getGPS().size() != 0) {
    ...
    private void save() {
        ...
        if (result == 1) {
            ...
            diseaseBaseInfo.setBHJBID(bhjbid);
            diseaseBaseInfo.setGYDWID(MyApplication.spUtils.getString("dqgydwid"));
            diseaseBaseInfo.setLDMC(lxInfo.getLDMC());//路段名称
            diseaseBaseInfo.setLXID(lxInfo.getLDID());
            diseaseBaseInfo.setCJSJ(diseaseNewTimeTxt.getText().toString());
            diseaseBaseInfo.setDCLX(diseaseNewToExamineTypeTxt.getText().toString());
            ...
            if (gcxmadapter.get() != null && gcxmadapter.get().size() > 0) {
                for (int k = 0; k < gcxmadapter.get().size(); k++) {
                    if (k == 0) {
                        ...
                        SL = replaceNullString(gcxmadapter.get().get(k).getJsgs());
                        ...
                        SL += "," + replaceNullString(gcxmadapter.get().get(k).getJsgs());
                        DW += "," + replaceDwNull(gcxmadapter.get().get(k).getJldw());
                        GGXMID += "," + replaceDwNull(gcxmadapter.get().get(k).getGcxmid());
                    }
                }
            }
            ...
            long resltSize = curingDao.updateBingHaiBase(diseaseBaseInfo);
            if (resltSize == 1) {
                isSaveOrUpdateData = true;
    ...
    private void initdate() {
        ...
        getdclxResult();
        getbhlxResult();
        String baseInfoJsonStr = getIntent().getStringExtra("baseInfoJsonStr");
        DiseaseBaseInfo diseaseBaseInfo = gson.fromJson(baseInfoJsonStr, DiseaseBaseInfo.class);
        bhjbid = diseaseBaseInfo.getBHJBID();
        diseaseNewTimeTxt.setText(replaceNull(diseaseBaseInfo.getCJSJ()));
        RoadSectionInfo ldinfo = curingDao.queryLdByLdid(diseaseBaseInfo.getLXID());
        lxInfo = ldinfo;
        if (ldinfo != null) {
            String qdzhstr = Utils.getZHMCByZH(ldinfo.getQDZH());
            String zdzhstr = Utils.getZHMCByZH(ldinfo.getZDZH());
            diseaseNewRoadLineTxt.setText(diseaseBaseInfo.getLDMC() + " " + qdzhstr + "-" + zdzhstr);
        } else {
            diseaseNewRoadLineTxt.setText(diseaseBaseInfo.getLDMC() + " " + Utils.getZHMCByZH(diseaseBaseInfo.getZH()));
        }
        ...
        if (zhStr.contains(".")) {
            ...
            diseaseNewPileNumberOneEdit.setText(zhone);
            diseaseNewPileNumberTwoEdit.setText(zhtwo);
        } else {
            diseaseNewPileNumberOneEdit.setText(zhStr);
        }
        String xsfxStr = diseaseBaseInfo.getWZFX();
        ...
        diseaseCustomEditItemBhlxTxtBtn.setText(diseaseBaseInfo.getBHLX());
        bhmc.setVisibility(View.VISIBLE);
        diseaseCustomEditItemBhmcTxtBtn.setText(diseaseBaseInfo.getBHMC());
        sgfs.setVisibility(View.VISIBLE);
        diseaseCustomEditItemSgfsTxtBtn.setText(diseaseBaseInfo.getSGFS());
        if (diseaseBaseInfo.getGGXMMC().indexOf(",") != -1) {
            String str1 = diseaseBaseInfo.getGGXMMC().replace(" ", "");
            String str2 = diseaseBaseInfo.getSL().replace(" ", "");
            String str3 = diseaseBaseInfo.getDW().replace(" ", "");
            String str4 = diseaseBaseInfo.getGGXMID().replace(" ", "");
            dbGCXMlist = Arrays.asList(str1.split(","));//名称
            dbSLlist = Arrays.asList(str2.split(","));//名称
            dbDWlist = Arrays.asList(str3.split(","));//名称
            dbGCXMIDlist = Arrays.asList(str4.split(","));//名称
        } else {
            dbGCXMlist.add(diseaseBaseInfo.getGGXMMC());
            dbSLlist.add(diseaseBaseInfo.getSL());
            dbDWlist.add(diseaseBaseInfo.getDW());
            dbGCXMIDlist.add(diseaseBaseInfo.getGGXMID());
        }
        if (dbGCXMlist != null && dbGCXMlist.size() > 0) {
            for (int i = 0; i < dbGCXMlist.size(); i++) {
                ItemBeanInfo itembean = new ItemBeanInfo();
                itembean.setGcxmid(dbGCXMIDlist.get(i));
                itembean.setXmmc(dbGCXMlist.get(i));
                itembean.setJldw(dbDWlist.get(i));
                itembean.setJsgs(dbSLlist.get(i));
        ...
        listImgUrl.addAll(Arrays.asList(diseaseBaseInfo.getTPDZ().split(",")));
        ...
        addPictureAdapter = new LookdiseasedbGrid(this, listPicture, listImgUrl, childPosition);
    ...
    private void getbhmcResult() {
        ...
        if (bhmcResultdb != null) {
            Log.e( "bhmcResultdb",bhmcResultdb.size()+"" );
            bhResult.clear();
        ...
        if (requestCode == CHOOSE_PICTURE_CODE) {//从相册获取照片
            if (intent != null) {
                ...
                if (position != -1 && listSelectPic != null && listSelectPic.size() > 0) {
                    final LookdiseasedbGrid addPictureAdapter = mapAdapter.get(position);
...
//                            .setTargetDir(savePathStr)                     // 设置压缩后文件存储位置
                            .setCompressListener(new OnCompressListener() { //设置回调
                                ...
                                public void onSuccess(File file) {
                                    ...
                                    Bitmap bitmap = Tooklkit.getImageThumbnail(imgPath, Tooklkit.dip2px(LookdiseasedbActivity.this, 480),
                                            Tooklkit.dip2px(LookdiseasedbActivity.this, 480));
                                    if (bitmap != null) {
            ...
            if (resultCode == -1) {
                if (!Utils.isNull(cameraPath) && childViewPosition != -1) {
                    ...
                    final LookdiseasedbGrid addPictureAdapter = mapAdapter.get(childViewPosition);
                    final ArrayList<Drawable> listPicture = addPictureAdapter.getListPicture();
                    final ArrayList<String> listImgUrl = addPictureAdapter.getListImgUrl();
...
//                            .setTargetDir(cameraPath.substring(0, cameraPath.lastIndexOf("/")))                        // 设置压缩后文件存储位置
    ...
    private void getlxResult(String gydwid) {
        lxResultdb = curingDao.queryLdByGydwid(gydwid);
        if (lxResultdb != null) {
            lxResult.clear();
            for (int i = 0; i < lxResultdb.size(); i++) {
                RoadSectionInfo sectionInfo = lxResultdb.get(i);
                String ldstr = sectionInfo.getLDMC();
                String ldqdstr = sectionInfo.getQDZH();
                String ldzdstr = sectionInfo.getZDZH();
                String resultqdStr = Utils.getZHMCByZH(ldqdstr);
                String resultzdStr = Utils.getZHMCByZH(ldzdstr);
                String resultNameStr = ldstr + resultqdStr + "-" + resultzdStr;
    ...
    AMapLocationListener locationListener = new AMapLocationListener() {
        ...
        public void onLocationChanged(AMapLocation aMapLocation) {
            if (aMapLocation != null) {//定位成功
                ...
                mPresenter.getZH(lxInfo.getLXID(), String.valueOf(longitude), String.valueOf(latitude), gydwid);
            } else {//定位失败
                Log.i("定位失败", "定位失败");
            }
            stopLocation();
