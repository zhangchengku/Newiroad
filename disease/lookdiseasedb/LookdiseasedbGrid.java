...
public class LookdiseasedbGrid  extends BaseAdapter {
    ...
    public LookdiseasedbGrid(Context context,
    ...
    public View getView(int position, View convertView, final ViewGroup parent) {
        ...
        if (showType == 1) {
            ...
            if (position == listPicture.size() - 1) {
                vh.pictureCloseImg.setVisibility(View.GONE);
                if (context instanceof LookdiseasedbActivity) {
                    vh.pictureImg.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            if (minePopupWindow == null) {
                                minePopupWindow = new MinePopupWindow((LookdiseasedbActivity) context, itemOnClick);
                            }
                            minePopupWindow.showAtLocation(parent, Gravity.BOTTOM, 0, 0);
                            Utils.hideInputWindow((LookdiseasedbActivity)context);
                        }
                    });
                }
            } else {
    ...
    public void getPicFromPhoto() {
        ...
        ((LookdiseasedbActivity) context).startActivityForResult(intent, CHOOSE_PICTURE_CODE);
    }
    ...
    public void getPicFromCamera() {
        ...
        ((LookdiseasedbActivity) context).setCameraPath(path);
        ((LookdiseasedbActivity) context).setChildViewPosition(childViewPosition);
        ((LookdiseasedbActivity) context).startActivityForResult(intent, CAMERA_CODE);
    }
